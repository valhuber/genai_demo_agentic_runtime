...
For **probabilistic (AI-driven) decisions**, see `docs/training/logic_bank_api_probabilistic.prompt` which describes `Rule.ai_decision()` for intelligent selection based on contextual reasoning (supplier optimization, dynamic pricing, route selection, etc.). These probabilistic rules work seamlessly with deterministic rules, with automatic audit trails and graceful fallbacks.
